...
@WebService(endpointInterface = "server.CampusServerInterface")
public class DorvalServerImpl implements CampusServerInterface{
...
@Override
    ...
    { BufferedWriter output = null;
	  ...
	  Date d1 = new Date();
	  String sStatus,sTimeSlot;
	  String sWestMountSentence = "11",sKirklandSentence = "11";
	  ...
	  { sTimeSlot = (String) itr.next();
	     if (hDate.get(date).get(roomNumber).get(sTimeSlot)==null)
    	   ...
    	   { sStudentId = hDate.get(date).get(roomNumber).get(sTimeSlot).substring(0, 7);
    	     if (sStudentId.substring(0,4).equals("DVLS"))
    	    	 ...
    	    	 sWestMountSentence = sWestMountSentence + "_" + sStudentId;
       ...
       if (sWestMountSentence.length()>2)
         sBookingCountStatus = otherServerUDP(9877,sWestMountSentence);
	...
	private String bookRoomDorval (String sStudentId,String roomNumber,String date,String timeslot)
	    { String sBookingId;
...
@Override
 ...
 { for(int i=0;i<sStudentIdArray.length;i++)
   { if (sStudentIdArray[i]!=null && (sStudentIdArray[i].equals(sStudentId)))
    ...
    private String getAvailableTimeSlotDorval(String date)
...
@Override
	...
	private String cancelBookingDorval(String sBookingID)
	{ BufferedWriter output = null;
	  DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
	  Date d1 = new Date();
...
@Override
    public String changeReservation (String bookingId,String newCampusName,String newRoomNo,String bookingDate,String newTimeSlot,String studentId)
  {
    String sBookingStatus=null,sBookingCancelStatus=null,sStatus="Not able to perform Action";

    if (newCampusName.equals("Dorval"))
      sBookingStatus = bookRoomDorval(studentId,newRoomNo,bookingDate,newTimeSlot);
    else if (newCampusName.equals("Westmount"))
    ...
    if (bookingId.charAt(0)=='D'&& sBookingStatus!=null&&sBookingStatus.contains("Room Booked."))
       sBookingCancelStatus = cancelBookingDorval(studentId+bookingId);
      ...
      {	 String sMessage="7_"+bookingId+"_"+studentId;
         ...
         else if (bookingId.charAt(0)=='W')
           sBookingCancelStatus = otherServerUDP(9877,sMessage);
      }
   ...
   if (sBookingCancelStatus!=null && sBookingCancelStatus.equals("Booking Cancelled"))
    ...
    { sStatus="Booking Slot Transfer Un-Successful";
      ...
      if (sNewBookingId.charAt(0)=='D')
          sBookingCancelStatus = cancelBookingDorval(studentId+sNewBookingId);
       else
         {	 String sMessage="7_"+sNewBookingId+"_"+studentId;
       	 if (sNewBookingId.charAt(0)=='K')
              sBookingCancelStatus = otherServerUDP(9876,sMessage);
            else if (sNewBookingId.charAt(0)=='W')
              sBookingCancelStatus = otherServerUDP(9877,sMessage);
         }
    }
   else
